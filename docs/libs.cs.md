# Hayo — Vestavěné funkce

Viz také: **[Syntaxe jazyka](syntax.cs.md)**
...
| `a - b` | rozdíl | Num |
| `a * b` | součin | Num |
| `a div b` | celočíselné dělení | Num → Int |
| `a mod b` | zbytek po dělení | Int |
| `Math.ceil a` | zaokrouhlení nahoru | Num → Int |
| `Math.floor a` | zaokrouhlení dolů | Num → Int |
| `Math.round a precision` | matematické zaokrouhlení | Num Int → Real |
...
| `Dict.values xs` | Dict | List\<a\> | seznam hodnot |
...
| Funkce | Argumenty | Výsledek | Popis |
|---|---|---|---|
| `Introspect.of src` | a | Str | název typu hodnoty jako řetězec |
| `Introspect.is src type` | a Str | Bool | vrátí True, pokud je hodnota daného typu |
